...
window.onload = function() {
  ...
  function displaySubtitle(cue, positionMS) {
    ...
    console.log(
      "webvtt timeOffset: " + timeOffset + " cue: " + JSON.stringify(cue)
    );
    ...
    if (timeOffset <= 200) {
      ...
      document.getElementById("subtitleContent").innerHTML = cue.text;
      ...
      subtitleTimer = setTimeout(function() {
        ...
        if (vttCueBuffer.length > 0) {
          displaySubtitle(
            vttCueBuffer[0],
            positionMS + cue.duration + timeOffset
          );
        } else {
  ...
  video.addEventListener("timeupdate", function() {
    ...
    var position = video.currentTime * 1000.0;
    ...
    if (
      displayTimer === undefined &&
      subtitleTimer === undefined &&
      vttCueBuffer.length !== 0
    ) {
      console.log("in timeupdate: position: " + video.currentTime * 1000.0);
  ...
  var HTML5Player = function() {
    ...
    this.keyUp = function() {
      if (this.dropDownListVisible) {
        ...
        this.components[this.currentPos] == this.closedCaptionButton
      ) {
  ...
  function toggleOverlay() {
    ...
    document.getElementById("logCheck").checked = !document.getElementById(
      "logCheck"
    ).checked;
  ...
  function onTuneFailedEvent(event) {
    console.log(
      "onTuneFailed event received: errorCode: " +
        event.code +
        " description: " +
        event.description
    );
  ...
  if (typeof window.AAMP_JSController !== "undefined") {
    ...
    window.AAMP_JSController.addEventListener(
      "playbackFailed",
      onTuneFailedEvent
    );
